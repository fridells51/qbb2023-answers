...
def plot_coverage_distributions(ax, paths):
    ...
    ax.bar(ont_coverages, ont_counts, alpha=0.5, label=labels[0])
    bis_coverages = read_bedgraph_coverage(paths[1])
    bis_coverages, bis_counts = np.unique(bis_coverages, return_counts=True)
    ax.bar(bis_coverages, bis_counts, alpha=0.5, label=labels[1])
    ax.set_title('Distribution of Coverages Across CpG Sites')
...
def calculate_pearson_r1(methylation_ont, methylation_bis):
    return np.corrcoef(methylation_ont, methylation_bis)[0, 1]

def plot_2d_histogram(ax, methylation_ont, methylation_bis):
    ...
    im = ax.imshow(histogram, extent=[x_edges[0], x_edges[-1], y_edges[0], y_edges[-1]], cmap='viridis', aspect='auto')
    plt.colorbar(im, ax = ax, label='log10(Count + 1)')
    ax.set_xlabel('Methylation Score ONT')
    ax.set_ylabel('Methylation Score Bisulfite')
    pearson_r = calculate_pearson_r1(methylation_ont, methylation_bis)
    ax.set_title(f'2D Histogram of Methylation Scores\nPearson R: {pearson_r:.3f}')
...
def plot_violin(ax, methylation_change_common):

    sns.violinplot(x='approach', y='methylation_change', data=methylation_change_common)

    ax.set_xlabel('Sequencing Type')
    ax.set_ylabel('Methylation Change (Tumor - Normal)')
    pearson_r = calculate_pearson_r(methylation_change_common)
    ax.set_title(f'Distribution of Methylation Changes\nPearson R: {pearson_r:.3f}')
...
plot_coverage_distributions(ax[0], [ont_path, bisulfite_path])
plot_2d_histogram(ax[1], methylation_ont, methylation_bisulfite)
plot_violin(ax[2], methylation_change_common)
plt.tight_layout()
